...
(function () {
    ...
    Tar.prototype.append = function (filepath, input, opts, callback) {
        ...
        if (typeof input === 'string') {
            input = new Uint8Array(utils.toUTF8Array(input));
        ...
        var filenamePrefix
        if(utils.toUTF8Array(filepath).length > 100) {
            var parts = filepath.split('/')    
            filepath = parts.pop()
            filenamePrefix = parts.join('/')
            var a = utils.toUTF8Array(filenamePrefix).length 
            var b = utils.toUTF8Array(filepath).length 
            if(a > 155 || b > 100) {
                throw "File name too long (at most 155 bytes for prefix (now "+a +") and 100 bytes for file name (now "+b+")): " + filenamePrefix + "/" + filepath
            }
        }
